...
### 設定
- `scalpRangeEnabled?: boolean`(config+⚙️UI トグル・**既定 true**・実験扱いの hint 必須)。false のとき AI に「range を出すな」と指示し、万一 range が返っても none 扱い。
...
- `SignalProposal`/`planFromSignal` に range 対応。`shouldFollowSignal` は direction==='range' でも(armed/新signalId/FLAT/建玉クリア)で追従。
- **両面発注**: upper・lower の2レッグ(逆方向)を発注。既存の**逐次OCO(cancel-the-loser)**を流用: どちらか約定 → 本ループが相方を id 取消。**checkSanity は「upper は現値超・lower は現値未満」を満たすときのみ発注**(外れは見送り=詰まらない・phase ゲート併用)。
