...
- Modify: `web/index.html`（key-groq 入力の隣）
- Modify: `web/components/settingsModal.ts`（el 81-83, placeholder 103-111, clear 348-350, body 366-371）
...
- el 型(81-83行)に `inputTavily: HTMLInputElement;` 追加し、el 構築箇所で `inputTavily: document.getElementById('key-tavily') as HTMLInputElement,` を追加（gemini と同じ場所）。
- placeholder(111行の後): 
  ```ts
