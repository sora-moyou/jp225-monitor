...
//   「分析用が動いていない」の切り分けで最初に知りたいのは「起動されたのか」なのに、それが観測できなかった。
//
...
/** ★分析用が「別インスタンスに pid ロックを取られていて起動を保留している」ことを表す目印。
 *  分析用サイドカー(書き手)と monitor のハートビート(読み手)で **同じ文字列** を使うための唯一の出どころ。
 *  ここに置くのは、両者が共通して依存できる一番軽いモジュールだから(分析用の重い依存を monitor に持ち込まない)。 */
