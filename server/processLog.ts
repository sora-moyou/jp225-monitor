...
//
// ■ ★規約は **ここ1か所** (重複を残さない)
//   分析用向けの server/generator/sidecarLog.ts は、この一般形の **薄い包み** に畳んである
//   (名前 'generator' と相乗り先を決めるだけ)。
//   畳む前は「listener を付けたら exit する」という同じ規約が2か所に分かれ、分析用側だけが
//   unhandledRejection を **記録して生き残らせていた**。分析用の名乗りは setInterval なので、
