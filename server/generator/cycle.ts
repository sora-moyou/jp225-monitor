 ...
 *  旧3(2026-08-17〜08-18): 'prompt-v1d'(= 質問文 v1d を投げた腕)。v1 から **「指値・ブレイク新規を
 *      現在値から最低50円離す」の記述だけ** を外したもの。設計書 §8 の移行順序の1番目=「層1(執行)を
 *      直す」の第一手として1日走らせ、同一 epoch+cycle の対応ペア209件で実測。
 *      結果: **主指標(両レッグ同幅率)が悪化**(current 76.4% → v1d 84.7%)。距離<50 の割合も悪化
 *      (58.4% → 66.4%・McNemar p=0.0037)、距離の中央値も悪化(40円 → 30円)、LC幅=55固着も悪化
 *      (36.1% → 44.3%)。⇒「最低50円離す」はコードの強制(10円)とは別に「置き方の誘導」として効いて
 *      いた。**消すと逆効果=不採用。この記述は残す。** よって v1d は腕から降ろす
 *      (変種の定義・語彙・omitMinDistance フラグ・テストは過去の台帳を読むために残す。使うのはここだけ)。
 *
 *  ★現(2026-08-18): 'prompt-v1e'(= 質問文 v1e を投げた腕)。**送る決済仕様は①と同じ 'current'**。
 *      v1e は v1 から **「指値・ブレイク新規の距離の上限(片レッグ200円/両レッグ幅400円)」の記述だけ**
 *      を外したもの(最低距離50円は上の実測により残す=動かす変数は1つ)。
 *      理由(仮説): v0.9.83 で「死んだ記述だけ」を消したら v1 自身が改善した可能性がある観測
 *      (距離<50 が 72〜77% → 58.4%)があるが日をまたいだ比較で交絡し断定できない。競合する距離の
 *      数値(200/400)が、生きている距離規則(節目からの5〜10円)を押しのけていた疑いを、この腕で測る。
