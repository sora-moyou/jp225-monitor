...
export const LC_BUFFER_NOTE =
  ...
  '★この「+5円」は 損切りの根拠に選んだ節目から わずかに離すだけ の緩衝(買いは下へ・売りは上へ)で、LC幅を作る量ではない。';
...
export function lcFloorRule(floorYen: number): string {
  return '★【最優先: 損切りの幅(無条件・例外なし)】各レッグは独立に次の不等式を満たすこと。\n'
    ...
    + '  ★出力前に必ず自分で引き算して各レッグの LC幅を数え、幅と損切り価格を rationale に書くこと(例「指値レッグ LC=(数値)円→損切り(価格)」を出したレッグぶん)。置き方だけを述べて幅を数えないのは不可。\n'
...
export const PLAN_BAD_EXAMPLES =
  ...
  + '  ✗①建値の隣: 買い stopEntry=R1+5 に stopLossForStop=R1(幅は数円)。節目の緩衝(5円)を幅そのものと読んだ形。→損切りの節目は S1/S2 を選び、幅は引き算の結果。[lcFloor=損切り幅が設定の下限より狭い]\n'
  + '  ✗②売りのブレイク新規の符号ミス: 売り stopEntry=S1−5 の損切りを stopEntry より下に置いた(根拠文の幅は正しかった)。幅が合っていても下側の損切りは建玉を守らず即約定する。→売りは2つとも「+」。[stopSide=損切りがエントリーの逆側]\n'
  + '  ✗③エントリーと損切りが同じ節目: 「指値はサポートの5円上、ブレイク新規はレジスタンスの0〜5円上」とだけ述べ、損切りも同じ節目を基準にした形。同じ節目の内と外の差は緩衝ぶん(数円)だけで幾何的に下限を満たせない。→買いの指値が S1 の内側なら損切りの基準は S2。[lcFloor]\n'
...
export function buildStrategySpec(i: StrategySpecInput): string {
  ...
  return [
    ...
    '- 節目への置き方(約定重視・節目ちょうどには置かない): 指値=狙う節目から 5〜10円 内側(現在値側)。ブレイク新規(stopEntry)=狙う節目の 0〜5円 外側(抜ける方向)。★選んだ節目に置いた結果が上の不等式に反するときは、まず不等式を満たす側の節目を選び直すこと。選び直しても適切な節目が無いときに限り、そのレッグを省く。',
