...
export const LC_DERIVATION_ORDER =
  ...
  + '★エントリーと損切りが同じ節目を基準だと幅は数円にしかならず必ず下限割れになる。'
  + 'LC幅は結果であって出発点ではない。先に幅(下限や上限の数値)を決めてから節目に当てはめるのは誤り。'
  + 'ゆえにレッグごと・場面ごとに幅が違ってよい(毎回ほぼ同じ幅なら、節目ではなく数値から逆算している)。';
...
export const LC_BUFFER_NOTE =
  '★この「+5円」は 損切りの根拠に選んだ節目から わずかに離すだけ の緩衝(買いは下へ・売りは上へ)で、LC幅を作る量ではない(幅は どの節目を選ぶか で決まる)。'
  + 'LC幅の下限に5円を足すという意味ではないし、**エントリー価格に5円を足し引きする**という意味でもない。';
...
export const SL_SIDE_RULE =
  ...
  + '  ★損切りをエントリーからの固定距離で決めてはならない: 建値の隣のティックに置く/両レッグを同じ幅にする/エントリーに5円を足し引きする/**エントリーの根拠にした節目をそのまま損切りの根拠にする**、はいずれも誤り(下の【導出の順序】に従う)。';
...
export function lcFloorRule(floorYen: number): string {
  return '★【最優先: 損切りの幅(無条件・例外なし)】各レッグは独立に次の不等式を満たすこと。\n'
    ...
    + '  ★出力前に必ず自分で引き算して各レッグの LC幅を数え、その数値を rationale に書くこと(例「指値レッグ LC=(数値)円 / ブレイク新規レッグ LC=(数値)円」)。置き方だけを述べて幅を数えないのは不可。\n'
    + `  ★${floorYen}円未満になったら、まず 損切りの根拠にする節目を もう一段外側へ選び直す。選び直しても届かないレッグだけを 対の損切りごと省く`
    + '(下限割れのレッグは取引されない=あなたの計画がその分だけ実行されないだけで、下限を守る責任は免除されない)。';
}
...
export function selfCheckNote(floorYen: number, ceilingYen: number): string {
  return '★【出力前の自己検算(必須)】出力前に limitEntry と stopEntry を refPrice と比較し、レッグごとに次の4点を確かめること。\n'
    ...
    + '  ③節目の別: 損切りの根拠にした節目が、エントリーの根拠にした節目と **別**(もう一段外側)か(同じ節目なら④は必ず足りない)。\n'
    + `  ④損切りの幅: |エントリー − 損切り| を実際に引き算し、${floorYen}円以上 ${ceilingYen}円以下 か。その数値を rationale に書いたか。\n`
...
- ★この LC 幅(下限・上限)は 指値レッグ・ブレイク新規レッグ それぞれ独立に 満たすこと。収まらないレッグは まず損切りの節目を選び直す(下限未満=一段外側 / 上限超=一段内側)。選び直しても収まらないレッグだけを 対の損切りごと省き、「指値のみ」または「ブレイク新規のみ」で取引する。両レッグとも収まらなければ direction:"none" で見送る。
- ${selfCheckNote(floorYen, ceilingYen)}
...
export function scalpJsonInstruction(
  ...
  return `最終的な回答は、次のスキーマに厳密に一致する JSON オブジェクトのみを出力してください(前後の説明文・コードフェンス・マークダウンは一切付けない)。\n` +
    ...
    rangeShape +
    `  "rationale": string,         // 判断理由(日本語)。none の時は見送り理由。★出したレッグごとに、自分で引き算した LC幅(例「指値レッグ LC=(数値)円」)を必ず含める\n` +
...
export function buildStrategySpec(i: StrategySpecInput): string {
  ...
  return [
    ...
    '- 節目への置き方(約定重視・節目ちょうどには置かない): 指値=狙う節目から 5〜10円 内側(現在値側)。ブレイク新規(stopEntry)=狙う節目の 0〜5円 外側(抜ける方向)。※この「外側」は抜ける方向の意味で、損切りの「外側」(建玉を守る向き)とは別物。★選んだ節目に置いた結果が上の不等式に反するときは、まず不等式を満たす側の節目を選び直すこと。選び直しても適切な節目が無いときに限り、そのレッグを省く。',
