...
export const LC_DERIVATION_ORDER =
  '★【導出の順序(必ずこの順)】①まず根拠にする節目/スイングを選ぶ → ②その節目の外側(買いは下・売りは上)にストップ位置を置き、'
  + '往復のダマシを吸収する緩衝として さらに5円 同じ向きへずらして損切り価格を決める → ③その結果として LC幅(|エントリー − 損切り|)が決まる。'
 ...
 *  下限に足すのではないこと に加えて、エントリーに足し引きするのでもないことを名指しで否定する。 */
export const LC_BUFFER_NOTE =
  '★この「+5円」は 節目/スイングから導いたストップ位置を さらに5円だけ 買いは下へ・売りは上へ ずらす緩衝である。'
  + 'LC幅の下限に5円を足すという意味ではないし、**エントリー価格に5円を足し引きする**という意味でもない(損切りまでの距離が5円になることは無い)。';
 ...
 *    損切り=エントリーの外側は"建玉を守る向き")ことも、ここで名指しで解く。 */
export const SL_SIDE_RULE =
  '★【最優先: 損切りの向き(無条件・例外なし)】損切りは必ず次の不等式を満たすこと。\n'
  + '  買い: stopLossForLimit < limitEntry ／ stopLossForStop < stopEntry\n'
  + '  売り: stopLossForLimit > limitEntry ／ stopLossForStop > stopEntry\n'
  + '  range も同じ(buy レッグは stopLoss < entry ／ sell レッグは stopLoss > entry)。\n'
  + '  ★ここでの「外側」は **建玉を守る向き**(買い=下・売り=上)。ブレイク新規で言う「節目の外側(=抜ける方向)」とは逆向きなので、決して同じ意味に読まないこと。\n'
  // ★末尾に「損切りは節目から導く」と書き足さない: 同じ文言の【導出の順序】が同じ本文の中に必ず入るため。
  + '  ★損切りをエントリーからの固定距離で決めてはならない: 建値の隣のティックに置く/両レッグを同じ幅にする/エントリーに5円を足し引きする、はいずれも誤り(下の【導出の順序】に従う)。';

/** ★v0.9.60: 出力前の自己検算(SSOT)。従来はエントリーの不等式しか検算させておらず、
 *  損切りの向き・幅は自己検算の対象外だった(=最多の脱落理由が自己検算をすり抜けていた)。同じ場所・同じ強さで3点を検算させる。 */
export function selfCheckNote(floorYen: number, ceilingYen: number): string {
  return '★【出力前の自己検算(必須)】出力前に limitEntry と stopEntry を refPrice と比較し、レッグごとに次の3点を確かめること。\n'
    + '  ①エントリーの向き: 上の不等式を満たすか。\n'
    + '  ②損切りの向き: 買いは 損切り<エントリー / 売りは 損切り>エントリー を満たすか(エントリーの隣のティックに置いていないか)。\n'
    + `  ③損切りの幅: |エントリー − 損切り| が ${floorYen}円以上 ${ceilingYen}円以下 か(${floorYen}円未満のレッグはコードが必ず落とす)。\n`
...
export function buildScalpQuestion(
  ...
  return (
    ...
    `そのレッグの LC が上限(≤${ceilingYen}円)に収まらないなら、(a)価格を SL 側に近づけて収めるか、(b)そのレッグを出さず「指値のみ」で取引する(stopEntry と stopLossForStop を出さない)。` +
    '対称に、指値レッグが構造上収まらないなら指値レッグを省いてブレイク新規のみにしてもよい。両レッグとも収まらなければ direction:"none" で見送ること。\n' +
...
- ★この LC 上限(≤${ceilingYen}円)は 指値レッグ・ブレイク新規レッグ それぞれ独立に 満たすこと。ブレイク新規(stopEntry)は現在値/節目から離れるほど LC が広がりやすい。そのレッグの LC が上限に収まらないなら、(a)価格を SL 側に近づけて収めるか、(b)そのレッグを省いて「指値のみ」で取引する(stopEntry / stopLossForStop を出さない=省略)。対称に、指値レッグが構造上収まらないなら指値レッグを省いてブレイク新規のみにしてもよい。両レッグとも収まらなければ direction:"none" で見送る。
- ${selfCheckNote(floorYen, ceilingYen)}
...
export function scalpJsonInstruction(
  ...
  const lcNote = `買いはエントリーより下/売りはエントリーより上・${lcWidth}・レッグ独立で${ceilingYen}円超は出さない・エントリーからの固定距離(建値の隣のティック等)で決めない`;
...
export function buildStrategySpec(i: StrategySpecInput): string {
  ...
  return [
    ...
    '- 節目への置き方(約定重視・節目ちょうどには置かない): 指値=狙う節目から最低5円(目安5〜10円)内側(現在値側)[買い=サポート+5〜10円 / 売り=レジスタンス−5〜10円]。ブレイク新規(stopEntry)=狙う節目の 0〜5円 外側(抜ける方向)[買い=レジスタンス+0〜5円 / 売り=サポート−0〜5円]。理由: 節目ちょうどだと指値は刺さらず・ブレイク新規はだまし(往復)に遭いやすい。※この「外側」は抜ける方向の意味で、損切りの「外側」(建玉を守る向き)とは別物。★選んだ節目に置いた結果が上の不等式に反するときは、まず不等式を満たす側の節目を選び直すこと。選び直しても適切な節目が無いときに限り、そのレッグを省く。',
