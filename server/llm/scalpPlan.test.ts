...
describe('scalp プロンプト 向きの構造化(v0.9.44)', () => {
  ...
  // ─── ★v0.9.93: 自己検算の① に「型」を持たせた(③ と同じ形) ────────────────────────
  //
  // ■ 実測(v0.9.92・proposals 26,734件を app_version で切って比較)
  //   エントリーが現在価格の逆側で落ちる geometry: 買い 3.1%→3.0% / ★売り 35.9%→**48.0%**(n=50)。
  //   失敗の型も入れ替わった: 旧 sell/limit/下 → 新 ★sell/stop/上 20件(ブレイク売りを上に置く)。
  // ■ 原因の読み(自己検算の非対称)
  //   ③ には「実際に引き算する」**作業** と「rationale に書く」**証跡** がある。
  //   ① は「上の不等式を満たすか」= 離れた参照を **確認するだけ** で、どちらも無かった。
  //   直近の違反数: ③の系(lcFloor)=0件 / stopSide(コードが強制)=0件 / ★①の系(geometry)=売り48%。
  //   このプロジェクトの記録「式の価値は答えでなく型。外すと符号ミスが3倍」に照らすと ① だけ型が無い。
  // ■ このテストが守るもの(次に誰かが「確認するだけ」へ戻したら赤くなる)
  //   ①-a 実際に大小比較させる作業がある / ①-b 結果を rationale に書き残させる /
  //   ①-c 離れた場所への参照(「上の不等式」)に戻っていない / ①-d 不等式を再掲していない(印字を増やさない)
  //   ★①-e **判定の相手(基準)がある** = 「真ん中に来る名前は refPrice」。
  //     これが無いと **証跡は残るがループが閉じない**: ベアで枠を取り違えても AI は正直に
  //     「refPrice < stopEntry」と書き、それで終わってしまう。③ が効いているのは「書かせる」からではなく
  //     **書いた答えを宣言済みの基準(下限〜上限 / lcWidthFor… の値)に当てている** から。① も同じ形にする。
  //   ★①-f **片脚の回で破綻しない** = 基準は「両レッグを出したなら」という条件つき。
  //     名前が2つしか並ばない回には「真ん中」が存在しないので、条件に当たらず何も要求されない
  //     (存在しない中央を探させない/否定文で「片脚のときは見るな」と書かない)。
  //   ②③ は1文字も変わっていない(1変数であることの固定)
  describe('★自己検算① は「実際に比較して書き残す」型(③ と同じ)', () => {
    const line1 = (t: string): string => {
      const l = t.split('\n').find(x => x.trim().startsWith('①エントリーの向き'));
      expect(l, '自己検算の①の行が無い(見出しを変えたなら、このテストも直すこと)').toBeTruthy();
      return l as string;
    };

    it('①-a 実際に大小比較させる作業がある(「満たすか」で終わらせない)', () => {
      for (const t of targets()) {
        const l = line1(t);
        expect(l).toContain('実際に大小比較し');
        expect(l).toContain('小さい順に並べて');
      }
    });

    it('①-b 結果を rationale に書き残させる(③ と同じ証跡)', () => {
      for (const t of targets()) expect(line1(t)).toContain('rationale に書くこと');
    });

    it('①-c 離れた場所への参照(「上の不等式」)に戻っていない = 否定対照', () => {
      for (const t of targets()) {
        expect(line1(t), '①が「上の不等式を満たすか」に戻っている').not.toContain('上の不等式');
        expect(line1(t)).not.toContain('満たすか');
      }
    });

    it('①-d 不等式を再掲していない(印字回数を増やさない)', () => {
      for (const t of targets()) {
        const l = line1(t);
        // 契約の不等式そのもの(refPrice を挟む形)は ★【最優先: 価格の向き】が1度だけ持つ。
        expect(l).not.toContain('< refPrice <');
        // 新しい数値も長さの指示も足していない。
        expect(l.match(/\d/g), `①に数値がある: ${l}`).toBeNull();
        for (const w of ['1行', '簡潔', '字以内']) expect(l).not.toContain(w);
      }
    });

    it('★①-e 判定の相手(基準)がある: 真ん中に来る名前は refPrice', () => {
      for (const t of targets()) {
        const l = line1(t);
        expect(l, '基準が無いと証跡は残るがループが閉じない').toContain('真ん中に来る名前は refPrice');
      }
    });

    it('★①-f 片脚の回で破綻しない: 基準は「両レッグを出したなら」の条件つき', () => {
      for (const t of targets()) {
        const l = line1(t);
        // 条件が基準より前に読める(片脚の回はそもそも条件に当たらない)。
        expect(l.indexOf('両レッグを出したなら')).toBeGreaterThanOrEqual(0);
        expect(l.indexOf('両レッグを出したなら')).toBeLessThan(l.indexOf('真ん中に来る名前は refPrice'));
        // 並びを書く要求のほうは片脚でも成立する形(「出したレッグの」)。
        expect(l).toContain('出したレッグの');
        // ★片脚のために否定文を足していない(「〜ない」で書き分けていない)。
        for (const w of ['片脚', '片レッグのときは', '見なくてよい', '不要']) expect(l).not.toContain(w);
      }
    });
...
// ─── ★「逆指値」語彙ガードの許容リスト(SSOT・下の2つの describe が共有する) ──────────────
//
// 許容するのは次の4つだけ:
//   (a) 用語の区別で **曖昧語として引用** している「逆指値」(鉤括弧つき)
//   (b) 決済(exit)の意味の「決済逆指値」
//   (c)(d) ★v0.9.89: ユーザーが逐語で書いた質問文Ｂ／対応表の注文名 **「Ｘの逆指値買い注文」「Ｙの逆指値売り注文」**。
//
// ★★★ (c)(d) は **必ず Ｘの / Ｙの まで含んだ完全形で許すこと**(ここを短くしない) ★★★
//   初版は '逆指値買い注文' / '逆指値売り注文' という **side 付きの複合語だけ** を許した。
//   その時の理由づけ「side が付いた複合語なので曖昧ではない(損切りは side を持たない)」は **誤り** である:
//     ・売り建玉の損切りは買い戻し = **まさに「逆指値買い注文」**
//     ・買い建玉の損切りは売り     = **まさに「逆指値売り注文」**
//   つまり許容2語は **損切り注文の正式名称そのもの** で、将来プロンプトに損切りの説明を書いたとき
//     「売り建玉の損切りは逆指値買い注文」「ストップは逆指値売り注文とする」
//   がガードを **無言で素通り** する(= fail-open)。v0.9.88 で slice(-1) の fail-open を直したばかりの型と同じ。
//   ★この二義性は v0.9.44 当時 **損切り逆位置171件** を生んだ張本人なので、狭い形だけを通す。
//   ★Ｘ/Ｙ は質問文Ｂが定義する **価格ラベル**(現在価格より上=Ｘ / 下=Ｙ)であり、損切りの説明文には現れない。
//     ゆえに「Ｘの/Ｙの まで含む完全形」= エントリーの意味に限定できる唯一の手掛かりになっている。
...
describe('★許容の狭さ(fail-open を作らない): 損切りの説明に紛れた「逆指値買い/売り注文」は赤のまま', () => {
  ...
  //   実例はすべて **実在しうる損切りの説明文**(売り建玉の損切り=買い戻し / 買い建玉の損切り=売り)。
  const COUNTER_EXAMPLES = [
    '売り建玉の損切りは逆指値買い注文',
    '買い建玉の損切りは逆指値売り注文',
    'ストップは逆指値売り注文とする',
  ];
  it.each(COUNTER_EXAMPLES)('損切りの説明「%s」は許容されない(赤)', (text) => {
    expect(stripReverseStop(text)).toContain('逆指値');
  });
  // 裸の語(従来からの本体)も引き続き赤。
  it.each(['逆指値', '逆指値注文', '逆指値で新規を入れる'])('裸の「%s」は許容されない(赤)', (text) => {
    expect(stripReverseStop(text)).toContain('逆指値');
  });
  // ユーザー逐語の完全形だけが通る(質問文Ｂと対応表が赤にならないための最小の穴)。
  it.each([
    '-買い目線の場合：Ｘの逆指値買い注文、Ｙの指値買い注文-',
    '-売り目線の場合：Ｘの指値売り注文、Ｙの逆指値売り注文',
    'ア）レンジ抜け：Ｘの逆指値買い注文、Ｙの逆指値売り注文',
    'Ｘの逆指値買い注文=stopEntry(+lcWidthForStop)',
  ])('ユーザー逐語の完全形「%s」は通る(緑)', (text) => {
    expect(stripReverseStop(text)).not.toContain('逆指値');
  });
});

describe('「逆指値」が新規(エントリー)の意味で残っていない(v0.9.44)', () => {
  // 許容は REVERSE_STOP_ALLOWED(SSOT・上の宣言を参照)。それ以外の裸の「逆指値」は新規の意味なので
  // ブレイク新規(stopEntry)へ置き換える。
  const strip = stripReverseStop;
...
describe('全生成箇所: 語彙ルール(「逆指値」が新規の意味で残っていない)', () => {
  // 許容は REVERSE_STOP_ALLOWED(SSOT・上の宣言を参照)。★Ｘの/Ｙの まで含む完全形だけを許す理由も同じ場所にある。
  const strip = stripReverseStop;
...
describe('v0.9.87: 価格の根拠にした節目(limitLevel / stopLevel)= 記録+表示', () => {
  ...
  // ★v0.9.88: 契約文の事実が **3度目** の更新を受けた(v0.9.85 記録専用 → v0.9.86/87 画面に表示
  //   → v0.9.88 コード導出のラベルが主で strategy は落ちることがある)。
  //   この it は「その時点の事実が書かれているか」を守るもので、**文言そのものを凍結するものではない**。
  //   表示の実装を変えたらこの it も一緒に動く(=実装だけ変えて嘘を残せない)。
  // ★v0.9.88: 契約文の事実が **4度目** の更新を受けた
  //   (v0.9.85 記録専用 → v0.9.86/87 画面に表示 → v0.9.88(1) 落ちる場合ができた
  //    → v0.9.88(2) **どのラベルが落ちるかは書かない**)。
  //   この it は「その時点の事実が書かれているか」を守るもので、文言そのものを凍結するものではない。
  //   表示の実装を変えたらこの it も一緒に動く(=実装だけ変えて嘘を残せない)。
  it('★契約文が v0.9.88 の事実を書いている(画面にも出る/形はコードが決める)', () => {
    const c = scalpStrategyContract();
    // ① 記録として集計に返す(v0.9.85 からの事実・不変)。
    expect(c).toContain('集計してあなたに返す');
    // ② 画面に出ることは書く(嘘にしない)。ただし **そのまま表示される** とは約束しない。
    expect(c).toContain('トレーダーの画面にも出るが、どう表示するかはコードが決める');
    expect(c).not.toContain('そのままトレーダーの画面に表示され');
    // ③ 注文に使わないのは事実なので残す(v0.9.85 からの事実・不変)。
    expect(c).toContain('注文の採否・価格・損切り幅には使わない');
    // ④ 以前の文言(「返すための記録である」で終わり=表示に触れない)は残っていない。
    expect(c).not.toContain('返すための記録である。');
    // ★事実の記述であって指示ではない: ラベルの選び方に触れる語を入れない(分布は測定対象)。
    for (const w of ['避け', '使わないこと', '選ばない']) expect(c).not.toContain(w);
  });

  // ★★★ 測定の汚染を防ぐ(この it が今回の裁定の本体) ★★★
  //   strategy の分布は **測定対象**(最頻ラベルは実測65%)。「どのラベルが画面から落ちるか」を
  //   契約文が名指しすると、AI がその選択肢を避ける経路ができて過去との比較が切れる。
  //   ★このブロックの外側には「押し目/戻り」「ブレイク新規」が既に7箇所以上あり、ラベルの偏りの
  //     第一容疑はもともとアンカーである。**その真上に語を足すのは最悪の位置**。
  //   ⇒ 「使われ方」の記述には、ラベル名も、コードが出す語も、1つも書かない。
  /** 「使われ方」の記述の見出し。★契約文でこの文言を変えたら、ここも必ず直すこと
   *  (直さないと下の indexOf 検査が赤くなる=無言では消えない)。 */
  const USAGE_MARK = '★この2つがどう使われるか';

  it('★「使われ方」の記述にラベル名/コードが出す語を1つも書かない(測定を汚さない)', () => {
    const c = scalpStrategyContract();
    // 「使われ方」の記述だけを取り出す(ラベル一覧そのものは上にあり、そちらは意図的に在る)。
    // ★★★ ここで indexOf の戻りを必ず検査する(fail-open を作らない) ★★★
    //   String.prototype.slice は **負の引数を末尾からのオフセット** として扱う。
    //   見出しを言い換えると indexOf は -1 になり、slice(-1) は「全文」ではなく **末尾1文字**。
    //   その1文字にラベル語は含まれないので、下の検査が **全部素通り** する
    //   = 測定汚染ガードが無言で消える(このプロジェクトが禁じている無言の失敗そのもの)。
    const at = c.indexOf(USAGE_MARK);
    expect(at, `契約文に見出し「${USAGE_MARK}」が無い(言い換えたなら、このテストの MARK も直すこと)`)
      .toBeGreaterThanOrEqual(0);
    const usage = c.slice(at);
    expect(usage.length).toBeGreaterThan(0);
    // ① AI が選ぶラベルの語を1つも出さない。
    for (const label of SCALP_STRATEGY_LABELS) {
      expect(usage, `使われ方の記述に「${label}」が出ている(選択肢を名指ししている)`).not.toContain(label);
    }
    // ② コードが画面へ出す語も1つも出さない。
    //    AI はこれらを自分では書かないので、知っても出力は1ビットも変えられない
    //    = 伝える価値はゼロで、アンカーになる危険だけが残る。
    for (const w of ['押し目買い', '戻り売り', 'ブレイク新規', '順張り', '逆張り', '押し目', '戻り']) {
      expect(usage, `使われ方の記述に「${w}」が出ている`).not.toContain(w);
    }
  });
