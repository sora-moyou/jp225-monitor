...
// ─── プロバイダ層 / 翻訳(別モジュールへ抽出済み) ───────────────────────
// プロバイダのライフサイクル・サーキットブレーカー・フォールバック・ビジョン判定は providers.ts、
// 英文ニュースの日本語訳は translate.ts に切り出した。既存 importer 互換のためここで再エクスポートする
// (バレル)。providers.ts の import 時副作用=プロバイダ構築 + `[LLM] enabled providers` ログは、
// この再エクスポート/直下の named import で providers.ts が読み込まれた時に一度だけ発火する。
export * from './providers.js';
export { translate } from './translate.js';

// openai.ts 内部(explain/chat/scalp-plan)がまだ使う分だけ named import する。
