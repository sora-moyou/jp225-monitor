...
//
// 何が壊れていたか(実取引PC monitor2 full v0.9.53 の実ログ 2026-08-03 00:04〜01:11):
//   00:04 60秒 → 00:05 300秒 → 00:11 1800秒 → 00:26 7200秒(2時間)。
//   A(実取引につながる default プール)は数分おきに 429 を踏むので、段が2時間まで伸びて
//   分析用は大半の時間止まり、標本が溜まらなかった。
//   ところが従属停止は **キーが共有かどうかを一切見ずに** 発火していた。ユーザーは4プロバイダ全てに
//   分析用専用キーを設定済み(検証も緑)で、上流のクォータは分かれている
//   = A の429は分析用のクォータについて **何も語らない**。存在しない危険のために止めていた。
//
...
/** default プールで A に 429 を踏ませ、そのあと分析用が通れるかを返す。 */
...
describe('★従属停止は「分析用が A と同じキーを使っているとき」だけ発火する', () => {
  beforeEach(() => {
    h.source = 'own';
    reloadProviders();
  ...
  it('★専用キー(own)なら A の429で分析用を止めない(存在しない危険で実験を殺さない)', async () => {
    h.source = 'own';
  ...
  it('出どころを解決できない(設定が壊れている等)なら止める=保護側に倒す', async () => {
    ...
    //   A は次プロバイダへ行けなくなる(実取引経路の破壊)。ここは 429 として素通しされねばならない。
    h.throws = true;
    try {
      expect(await generatorAllowedAfterDefault429()).toBe(false);
    } finally {
