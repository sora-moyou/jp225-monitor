...
//      ★v0.9.104 追加: 「損切幅は55円」(**り なし**)と「（LC幅55円）」(括弧が語句全体を囲む形)。
//      ★足した理由(2026-08-30 リーダー裁定): ①B の質問文は **ユーザーが指定した文面** なので
//        測定の都合で黙って言い換えない ②監査は RECORD-ONLY なので読める形を増やしても
//        判定・採否・価格は1バイトも変わらない(**足すだけ**) ③B の質問文は別タスク(TP)で
//        これから変わるので、同じ版で2つ動かすと効果を分離できない
//        (v0.9.98 で実際に踏んだ型: データ削減とプロンプト変更を同じ週に動かして分離不能になった)。
...
const WIDTH_RE = new RegExp(
  [
    ...
    // ★v0.9.104: 「り」を任意にする(`損切幅` = **現行プロンプトが実際に書かせている語**)。
    //   実測: v0.9.102 の根拠文 323件中 **181件** がこの形で、従来は1件も読めなかった。
    //   ★`損切り幅` は従来どおり読める(り? を付けただけ=既に読めていた形を1つも壊さない)。
    `損切り?幅\\s*(?:は|が|＝|=|:|：)?\\s*${WIDTH_NUM}\\s*円`,
    // ★v0.9.104: 括弧が **数値ではなく語句全体** を囲む形(「（LC幅60円）」)。実測 12件。
    //   ★従来の2枝は「LC幅 の **後ろ** に括弧」(「LC幅は(55)円」/「LC幅(55)」)しか読めない。
    //   ★助詞は **入れない**。「（LC幅は60円）」は内側を既存の枝が同じ値で読むので、枝を増やして
    //     取り合いにすると 当たる位置だけが1文字動き、見出し(レッグ)の割り当てが静かに変わりうる。
    //   ★偽陽性ガード (?!\d) は据え置き=「（LC幅65780円）」(価格)から先頭3桁を拾わない。
    `[（(]\\s*LC幅\\s*(\\d{1,3})(?!\\d)\\s*円?\\s*[)）]`,
