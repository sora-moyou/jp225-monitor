...
//
//   'default'   = 実弾(A)につながる既存の全経路。シグナルエンジン(server/signalTrade/engine.ts)・
//                 POST /api/scalp-plan の既存呼び出し・手動診断。**挙動は一切変えない**。
//   'generator' = 分析用の「提案生成器」(別プロセス・2分間隔でチャート画像つき LLM を叩く実験系)。
//                 実験系が実弾の経路を劣化させてはならない、というこのプロジェクトの一貫した原則により、
...
//   モジュール単位のシングルトンだった circuitOpenUntil が共有される(=生成器の429で実弾が最大8時間止まる)。
 ...
 *  - それ以外 → **エラー(400)**。
 *
 *  ★未知の値を黙って 'default' に倒さない理由: 生成器が caller を打ち間違えた時、それを default 扱いすると
 *    実験系が実弾のプール・予算・backpressure をそのまま使ってしまう(=この機能が守ろうとしたものが壊れる)。
