...
// v0.3.34: 同方向は 30 分クールダウン。ただしクールダウン中でも「逆方向」は、発火時の起点価格を
// 越えて価格が戻った場合(上発火→起点割れ / 下発火→起点超え)だけ許可する。どれか1つでも
...
export function canFire(symbol: string, dir: Dir, price: number, now: number = Date.now()): boolean {
  ...
  if (!rec) return true;
  if (now - rec.at >= COOLDOWN_MS) return true;
