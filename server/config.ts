// v0.3.32〜: 監視銘柄 INSTRUMENTS は core/instruments.ts に移設済み(STEP 2 リファクタ)。
...
【個別銘柄の扱い】
- 指数寄与の大きい値がさ株(ファーストリテイリング・アドバンテスト・ソフトバンクG・東京エレクトロン・ディスコ等)の大幅な値動きは、日経先物急変の真因になり得る。方向が一致するなら理由として挙げてよい。
- ただし指数寄与の小さい個別株や、方向が逆/無関係な個別株は理由にしない。また同じ古い材料を繰り返さず、直近(前回アラート以降)の新しい材料を優先する。
- 【重要・取引時間】東証個別株(.T)は 9:00-15:30 JST のみ取引。**昼休み・夜間・早朝(=先物のNightセッション等)は前回終値で固定され動かない**。日経先物の夜間・場外の急変の理由に個別株の値動きを持ち出さない(その時刻に動いていないため)。個別株を連動材料にしてよいのは東証の立会中(9:00-15:30)に限る。
