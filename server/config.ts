...
【理由に使わない(除外)】
- 個別銘柄の値動き(例: ファーストリテイリング・ディスコ・SMC・東京エレクトロン等の個別株が上昇/下落した、という個別株ニュース)を、日経先物急変の理由として挙げない。指数は多数銘柄の総和で、個別株の上げ下げは原因でなく結果のことが多い。
- 説明は「指数・先物全体 / 他資産(米株指数 NQ・YM、米10年金利、ドル円、原油) / セクター全体 / マクロ・金融政策・地政学イベント」に絞る。値がさ株に触れるのは「指数寄与が大きく相場全体を動かした」文脈に限り、個別物色の材料としては列挙しない。
