...
// ─── 決済仕様の「名前付き変種」 ──────────────────────────────────────────────
//
// 分析用の分析用は「同じ相場に、現行の決済仕様を教えた AI と 候補の決済仕様を教えた AI」を
...
//   - 'current'     … 実運用の現行仕様(既定)。
//   - 'candidate-a' … 分析用の候補。利益ロックの **第1段** の発動値と床値だけを差し替え、
//                     それ以外の段は現行のまま。**その2つの実数値はここに書かない**
//                     (候補値も現行値も ./private.ts が持つ。名前 → 数値の解決はプロセス内で閉じる)。
//
// ★なぜ「候補だから公開してよい」ではないのか(消すとまた書かれるので理由を残す):
//   このファイルは追跡下で、lite ビルドは公開リポ(sora-moyou/jp225-monitor)へ push される。
//   候補値は現行のフェーズ式ラチェットと **同じ格子の上に置いた値** なので、候補を1組見せることは
//   現行の段の桁・刻み・設計思想をそのまま見せることになる(1段目の発動/床が分かれば以降の段も推定できる)。
//   「候補だから現行値ではない」は公開してよい理由にならない。**このファイルに決済の数値は一切書かない**。
 ...
 *  - それ以外 → **エラー(400)**。
 *
 *  ★未知の値を黙って 'current' に倒さない理由: 分析用②が変種名を打ち間違えた時にそれを現行として
 ...
 *  - 'ratchet'    … ラチェット床(保有中に自分で決める水準)の変更。実取引でも実現可能だった。
 ...
 *
 *  ★なぜ記録するか: 分析用②は「候補の決済仕様を教えた AI」の提案を作り、影ラダーは「候補の決済仕様で
...
//   指紋がプロセスの寿命のあいだ固定され、記録(DB の版台帳・取引に刻む版・分析用の runs)が
...
export async function loadExitImpl(): Promise<'private' | 'simple'> {
  ...
  try {
    ...
    //   影(記録専用)の不具合が実取引につながる決済を変えてはならないので、影の失敗は影だけで閉じる。
    try {
