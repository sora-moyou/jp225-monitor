...
//   ④ 実弾に触れない・遅らせない — このモジュールは **DB を一切触らない**(行はバッファに積むだけ)。
 ...
 *    - 紙エンジンには引け決済が無く建玉は翌場へ持ち越す(実績の最長保有=206分)。ただし 206分は
...
/** 影の供給元。'generator'=分析用の提案生成器 / 'live-A'=実弾系 A が実際に武装した提案。 */
...
export interface ShadowRow {
  /** 生成器の設定版。name↔決済パラメータの対応はこの版の中でだけ意味を持つ。 */
  epoch: string;
  source: ShadowSource;
  /** 提案の識別子(live-A は signalId・生成器は生成器の採番)。同じ提案の影は同じ値を共有する。 */
  proposalId: string;
