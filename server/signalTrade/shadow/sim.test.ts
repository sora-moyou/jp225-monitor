...
// ★T0 は **実在する立会時間** でなければならない(2026-09-05 に 1_700_000_000_000 から差し替え)。
//   旧値は JST 2023-11-15 07:13:20 = 夜間引け(6:00)と日中寄り(8:45)の **あいだ** = 立会外だった。
//   advance() は 2026-09-05 から「武装した時刻と現在が同じ立会セッションか」を見て未約定ブラケットを
//   取り消す(引け後に武装したブラケットが引け後の価格で約定する事故の対策)。立会外の時刻を起点にすると
//   影は全部その場で取り消され、この節のどの検証も成立しない。
//   ★新値 = JST 2023-11-15(水) 10:00:00 = 日中セッション。このファイルが使う最大オフセットは
//     T0 + ARMED_TIMEOUT_MS(15分)なので、引け(15:45)まで 5時間45分の余裕がある。
//   ★assertions は1つも変えていない(T0 は「ある瞬間」を指す記号として使われているだけ)。
//     実証: **新 T0 + 修正前の本番コード → 16 passed / 16**(評価者の実測)= 失ったものはゼロ。
//   ★★申し送り: この差し替えで、このファイルは **JST の取引カレンダーへの依存を新たに持った**。
//     2023-11-15 は休場表(DERIV_NON_TRADING は 2026/2027 しか持たない)の外の平日なので現状は安全だが、
//     `core/session.ts` のセッション定義(8:45/15:45/17:00/6:00・休場表・週末ロジック)を触ると、
//     **この16本が「影とは無関係な理由で」赤くなり得る**。そのときは T0 を別の立会時間へ動かすこと
//     (アサーションではなく T0 を疑う)。
const T0 = Date.UTC(2023, 10, 15, 1, 0, 0);   // = 1_700_010_000_000
const buyPlan = {
