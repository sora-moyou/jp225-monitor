...
//
// 別プロセス(分析用)は monitor の中で決済仕様がどう解決されたかを知る手段が無かった。
// private が不在なら 'candidate-a' のプロンプトは公開フォールバック(数値なし)になり、
// 2本の分析用がほぼ同じ質問を投げる=実験は何も測っていないのに、記録には
...
export async function exitStatus(now: number = Date.now()): Promise<ExitStatus | { error: string }> {
  try {
    ...
    // ★実装が確定していないあいだは null(= まだ言えない)。前提検証(分析用)は指紋が無い期を
...
/** 分析用の台帳の死活(最終記録が何分前か)。**表示専用**なので、ここで例外を出して
...
function readGeneratorLedger(): ReturnType<typeof readGeneratorLedgerStatus> {
  try {
    ...
    console.warn('[status] 分析用の台帳を読めません:', e instanceof Error ? e.message : String(e));
...
export async function statusHandler(_req: Request, res: Response): Promise<void> {
  // ★公開版(lite)は分析用を持たない=その状況を返しても意味が無い(存在しない機構の
  ...
  const generator = isAnalysisEnabled()
    ...
    ? {
      ...
      // ★分析用サイドカーが有効化されているか(設定・既定 false)。
      //   「動いていない」の理由が **無効だから** なのか **止まったから** なのかは別物なので、
      //   台帳の死活(generatorLedger)とは **別のフィールド** で出す。
      //   ★台帳と混ぜない理由はもう1つある: 台帳は分析用が書くもので、monitor が知っているのは設定。
  ...
  res.json({
    ...
    // ★既存フィールドは不変: llm は従来どおり **default プール**(実取引につながる経路)の状態。
