...
/** ③の閾値: 左脚の BW ≥ BWhigh × これ。仕様 第6章は 0.9 だったが **2026-08-16 に 0.5 へ**。
 *
 *  ★なぜ緩めたか(用途が「売買」から「アラート専用」に確定したため):
 *    仕様⑥はネック逆指値でのエントリーを含んでいたが、基礎データ8.61年での検証(348セル)で
 *    **ダブル形状を起点にした機械売買を全否定**した。建値をネック抜け(60セル)/検知直後(72セル)/
 *    谷そのもの(216セル)と3通り変えても、スプレッド10円込みで乱択に勝つセルは一つも無い。
 *    決め手は局所プラセボ対照 — 局面・ボラ・向き・SL距離・TP距離を固定したまま建値だけを谷から
 *    ±15/±30円ずらすと、**16ブロック中「本物の谷」が勝率最高だったのは2つだけ**(p=0.5〜0.98)、
 *    逆に7ブロックでは谷が期待値最悪。前の安値は15円ずれたただの価格と区別できない。
 *    → 売買規則は落とし、検知は**アラート専用**になった。
 *    アラートの評価軸は損益ではなく「使える頻度で出るか」だけになる。
 *
 *  ★0.5 を選んだ根拠(実データ 2025-12-29〜2026-07-30 / 1分足181,577本 = 213.5日・本番と同じ配線で
 *    5分ごとに detectSwingDouble を評価し breakout を数えたもの):
 *
 *  | 比率 | breakout発火 | 相異なるW | bottom / top | 初回 → 最終 |
 *  |---|---|---|---|---|
 *  | 0.9 | 96 | 2 | 96 / **0** | 07-14 → 07-27 |
 *  | 0.7 | 100 | 3 | 100 / **0** | 03-05 → 07-27 |
 *  | **0.5** | **122** | **6** | 100 / **22** | **02-06 → 07-27** |
 *  | 0.3 | 269 | 8 | 222 / 47 | 02-06 → 07-27 |
 *
 *    **0.5 は「ダブルトップが1件でも出る最初の比率」**。0.9/0.7 では7か月間トップがゼロで、
 *    上側の検知が丸ごと死ぬ。0.3 まで緩めると下側が倍増して性格が変わるので 0.5 で止めた。
 *  ★ただし比率は主因ではない: 絞っているのは ①と③ の**同時成立**。旧測定(帯なし)は 182件/W38 なので、
 *    0.5 でも W は 38→6 までしか戻らない。件数が足りなければ、ここを更に動かすより
 *    ①(右脚がバンドの内側)の見直しの方が効く(トップの①の単独通過率は 22.4% と特に低い)。
 *
...
export const DOUBLE_BW_PEAK_RATIO = 0.5;
