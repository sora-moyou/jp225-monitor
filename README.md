...
- 📊 **4 銘柄リアルタイム監視** (NIY=F, NQ=F, YM=F, JPY=X) を 2 秒間隔で
...
- 🔄 **自動アップデート** (起動 5 秒後にチェック、新版あれば右上トースト / 設定画面「更新」の「更新をチェック」でアプリ本体と基礎データを同時に手動確認も) — v0.3.3 でインストール時ファイルロック問題解消
...
左から: 価格カード (日経225先物) → TradingView チャート (3 分足) → AI チャット。右に急変アラート 3 件 (15 分文脈 + LLM 説明) と日英ニュース。トップバーには相関リーダー、API 稼働ドット、ログ・設定ボタン。
