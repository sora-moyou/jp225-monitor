...
//   ★v0.9.104(2026-08-30)に足した2形: 「損切幅は60円」(り なし)/「（LC幅60円）」(括弧が語句全体)。
//     ★これは **剥がす量が増える** 方向の変更。規範「AI の判断理由の本文は絶対に落とさない」は不変で、
//       落ちるのは従来どおり isLcOnlyFragment が「実質の語が1つも残らない」と判定した断片だけ。
//     ★実測(複製DB・v0.9.92 以降の根拠文 全期間): 表示が変わった根拠文は下の報告のとおり。
...
const WIDTH_SRC = [
  ...
  // ★v0.9.104: 監査側(rationaleLc.ts)と **同時に** 足した2形。写しなので片方だけ直すと
  //   core/rationaleDisplay.test.ts の突き合わせが落ちる(=それが正しい働き)。
  //   ① `損切幅`(り なし)= 現行プロンプトが実際に書かせている語。実測 v0.9.102 の 323件中 181件。
  `損切り?幅\\s*(?:は|が|＝|=|:|：)?\\s*${WIDTH_NUM}\\s*円`,
  //   ② 「（LC幅60円）」= 括弧が **数値ではなく語句全体** を囲む形。実測 12件。
