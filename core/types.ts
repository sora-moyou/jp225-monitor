...
export interface Level {
  ...
  labels: string[];
  /** ★2026-08-26: この水準に寄与した候補の **種別**(重複なし)。
   *  ★なぜ labels ではなく kinds が要るか: labels は表示用の日本語で、クラスタでは `・` 連結されて
   *    増えていく。「ピボットか計算値か」を日本語の部分一致で判定すると、ラベル名を1文字直した瞬間に
   *    黙って壊れる。**種別は最初から内部に在る**(server/levels.ts の Cand.kind)ので、そのまま出す。
   *  ★用途: 5円ずらし(core/pivotNudge.ts)が「実際に注文が出た値段(ピボット)」だけを対象にする。
   *  ★**任意**にしてある: 既存の記録・テストのフィクスチャ・外部から来る Level を壊さないため。
   *    ★無いときは「ピボットと確認できない」= **ずらさない**(安全側。勝手に価格を動かさない)。 */
  kinds?: string[];
