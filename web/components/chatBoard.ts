...
const PRESETS: Preset[] = [
  { key: '1',
    ...
    prompt: '今の日経225先物について、15〜60分(約30分足)の時間軸でのトレンド方向と上値/下値のメドを、マークダウンの見出し(#)や太字(**)を使わずプレーンテキストで、次の形式で簡潔に答えて。\n1行目「15〜60分の時間軸では○○トレンドにあると推察」(○○は上昇/下降/レンジ)。\n2行目以降、根拠を「*」始まりの箇条書き(現値と中期15分平均・長期60分平均の位置 / 30分・60分変化率)。\n次の行に「上昇目途」と書き、その下にテクニカル要約の『上昇目途候補』を近い順に「*+〇〇円(根拠)」形式で列挙。\n次の行に「下落目途」と書き、その下に『下落目途候補』を同様に列挙。\n候補に『：トレンド転換』『：トレンド加速』が付いたものはその注記もそのまま残す。距離・根拠はテクニカル要約の数値をそのまま引用し、自分では計算しない。' },
  { key: '2',
    label: '急変の理由を詳しく',
    prompt: '直近で起きた急変の理由を、ニュース・他資産の動き・テクニカルの観点から詳しく説明して。マークダウンの見出し(#)や太字(**)は使わずプレーンテキストで、結論を1行で述べ、続けて根拠を「*」始まりの箇条書きで簡潔に。' },
];
