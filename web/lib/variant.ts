...
export interface VariantElements {
  ...
  // ★分析用の fieldset = **2つ目の API キー欄と日次予算欄**(#generator-keys-fieldset)。
  //   lite は分析用そのものを走らせない(server 側 analysisGate)ので、キーを入れる場所も出さない。
...
export function applyVariantVisibility(variant: Variant, els: VariantElements): void {
  ...
  const targets = [
    ...
    els.generatorKeysFieldset,     // ★分析用の2つ目の API キー欄/日次予算欄は lite で非表示。
