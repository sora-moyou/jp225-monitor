...
const NK_META: InstrumentMeta = {
  symbol: 'NIY=F', labelJa: '日経225', labelEn: 'Nikkei',
...
describe('ChangeDetector', () => {
  it('returns empty array when buffer too small (single sample)', () => {
    ...
    expect(d.feed(makePrice('NIY=F', 38000, 0))).toEqual([]);
  });

  it('fires slope alert when 30s move >= threshold', () => {
    const d = new ChangeDetector([NK_META]);
    d.feed(makePrice('NIY=F', 38000, 0));
    const alerts = d.feed(makePrice('NIY=F', 38000 * (1 - 0.0011), 25_000));
  ...
  it('fires magnitude alert when 5min move >= threshold but slope didnt', () => {
    ...
    d.feed(makePrice('NIY=F', 38000, 0));
    d.feed(makePrice('NIY=F', 38000 * 1.001, 60_000));    // +0.1% / 1min  → 30sでは超えない
    d.feed(makePrice('NIY=F', 38000 * 1.002, 120_000));   // 累積+0.2%
    const alerts = d.feed(makePrice('NIY=F', 38000 * 1.0031, 240_000));
    expect(alerts.length).toBe(1);
  ...
  it('does not fire when both within threshold', () => {
    const d = new ChangeDetector([NK_META]);
    d.feed(makePrice('NIY=F', 38000, 0));
    expect(d.feed(makePrice('NIY=F', 38010, 10_000))).toEqual([]); // +0.026%
  });

  it('does not double-fire within cooldown', () => {
    const d = new ChangeDetector([NK_META], { cooldownMs: 60_000 });
    d.feed(makePrice('NIY=F', 38000, 0));
    d.feed(makePrice('NIY=F', 38000 * 0.9985, 20_000)); // -0.15% / 20s → slope発火
    const second = d.feed(makePrice('NIY=F', 38000 * 0.998, 30_000));
  ...
  it('drops samples older than the 5-minute window', () => {
    ...
    const alerts = d.feed(makePrice('NIY=F', 38000 * 0.997, 6 * 60 * 1000));
